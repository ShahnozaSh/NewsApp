...
class Parser {
    ...
    func getImagePosts(completion: @escaping ([PostsImage]) -> ()) {
        DispatchQueue.main.async {
            URLSession.shared.dataTask(with: PostsImage.urlRequest) { [weak self] data, response, error in
                self?.showError(error)
                guard let data = data else { return }
                
                do {
                    let image = try JSONDecoder().decode([PostsImage].self, from: data)
                    completion(image)
                } catch {
                    self?.showError(error)
                }
